...
{
	// Used to give the chunks their ID andto taget chunks to create them
	int i = 0;
...
void WorldManager::UpdatePhysics(Player* player)
{
	...
	for (int i = 0; i < m_ChunkList.size(); ++i)
	{
		...
		if (c != NULL)
		{
			...
			{
				player->SetPosition(glm::vec3(player->GetPosition().x, player->GetPosition().y - 0.5f, player->GetPosition().z));
			}
